...
# Reads the CSV files
women_age_groups <- read_csv('data/fi/women_age_groups_by_year.csv')
births_long <- read_csv('data/fi/births_by_mother_age_group_year.csv')
...
ggplot(final_data, aes(x = Year, y = birth_rate, color = Age)) +
  ...
  geom_text(aes(label = ifelse(Year >= 2020, paste0(round(deviation, 1), "%"), "")),
            vjust = -1, size = 6) +
  labs(title = "FI - Birth Rates per 1000 Women in Age Group and Deviation from Linear Trend (2013-2019)",
       x = "Year",
