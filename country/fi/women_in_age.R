...
# Reads the CSV file
file_path <- "data/fi/women_age_groups_by_year.csv"
...
write.csv(total_by_year, 'data/fi/women_in_age_by_year.csv')
print(total_by_year)
...
ggplot(total_by_year, aes(x = Year, y = Total)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  labs(title = "FI - Total Number of 15-44 Women (2013-2023)",
       x = "Year",
...
ggplot(data_long, aes(x = Year, y = Count, color = Age)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  labs(title = "FI - Evolution of 15-44 Women by Age Group (2013-2023)",
       x = "Year",
       y = "Number of Women",
       color = "Age Group") +
  theme_minimal() +
