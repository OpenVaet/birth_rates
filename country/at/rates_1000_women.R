...
# Reads the CSV files
births_data <- read_csv('data/at/births_by_year.csv')
women_data <- read_csv('data/at/women_in_age_by_year.csv')
...
ggplot(data, aes(x = year, y = birth_rate)) +
  ...
  geom_text(aes(label = ifelse(year >= 2020, paste0(round(deviation, 1), "%"), "")),
            vjust = -1, size = 6) +
  labs(title = "AT - Birth Rates per 1000 Women in Age and Deviation from Linear Trend (2013-2019)",
       x = "Year",
