...
# Births data
years <- 2013:2023
births <- c(89325, 92440, 92449, 93820, 92005, 90069, 89429, 92664, 93211, 88442, 85209)

data <- data.frame(year = years, births = births)
data$births <- as.integer(data$births)

# Prints the yearly births.
write.csv(data, 'data/hu/births_by_year.csv')
...
ggplot(data, aes(x = year, y = births)) +
  ...
  geom_text(aes(label = ifelse(year >= 2020, paste0(round(deviation, 1), "%"), "")),
            vjust = -1, size = 6) +
  labs(title = "HU - Births and Deviation from Linear Trend (2013-2019)",
       x = "Year",
       y = "Number of Births",
